...
import {ProgressCallbackData, RenderParams, ResultCallbackData, SaymotionClient, Status} from '../src';

const OUTPUT_DIR = './output';

async function main(): Promise<void> {
    function onProgress(data: ProgressCallbackData): void {
    ...
    async function onResult(data: ResultCallbackData): Promise<void> {
        if (data.result) {
    ...
    if (!jobs.length) {
        ...
        console.log('Run basic_usage.ts first to create a text2motion job.');
        return;
    }
